...
@section('content')
        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                        ...
                        <div class="row align-items-md-center">
                            ...
                            <div class="col-md-7">
                                <div class="d-flex justify-content-md-end align-items-center">
                                    <span class="d-block text-muted pr-2">
                                        <i class="far fa-eye"></i>
                                        {{ $post->view }}
                                    </span>
                                    <span class="d-block text-muted pr-2">
                                        <a class="text-muted" href="{{ asset('author/' . $post->Author->username) }}">
                ...
                <div class="row justify-content-sm-between align-items-sm-center space-1 ">
                    <div class="col-sm-7 mb-2 mb-sm-0">
                        <div class="d-flex align-items-center">
                            ...
                            <a class="btn btn-xs btn-icon btn-ghost-secondary rounded-circle mr-2"
                    ...
                    @if (Auth::check())
                        <div class="col-sm-2 text-sm-right">
                            <a class="btn btn-xs btn-icon btn-soft-secondary rounded-circle reporting" data-toggle="modal"
                                data-target="#exampleModalTopCover" href="javascript:;" data-toggle="tooltip"
                ...
                <div id="exampleModalTopCover" class="modal fade" tabindex="-1" role="dialog"
                    aria-labelledby="exampleModalTopCoverTitle" aria-hidden="true" style="display: none;">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-top-cover-avatar">
                                <img class="avatar avatar-lg avatar-circle avatar-border-lg avatar-centered shadow-soft"
                                    src="{{ asset('frontend/assets/svg/brands/front.svg') }}" alt="Logo">
                            </div>
                            ...
                            <div class="modal-body content-report">
                                ...
                                <div class="form-group">
                                    <form action="{{ route('reporting.store') }}" method="post">
                                        ...
                                        <textarea id="my-textarea" class="form-control" name="reason" rows="3"
                                            required></textarea>
                                </div>

                                <button type="submit" class="btn btn-sm btn-primary float-right send-report">Báo
                                    cáo</button>
                                </form>
                            </div>
                            ...
                            @if (Auth::check())
                                @if (App\Models\Followers::where(['author_id' => $post->author_id, 'user_id' => Auth::user()->id])->exists())
                                    <form action="{{ route('followers.store') }}" method="POST">
                                        @csrf
                                        <h3 class="mb-0">
                                            <a href="{{ asset('author/' . $post->Author->username) }}">
                                                {{ $post->Author->fullname }}
                                            </a>
    ...
    <div class="card bg-img-hero bg-navy text-white text-center p-4 my-4 w-md-60  mx-md-auto "
        ...
        <h4 class="text-white mb-3">Bài viết có thú vị không? Nếu có hãy đăng ký để nhận thông báo khi có bài viết của thác
            giả này.</h4>
            ...
            <div class="js-form-message">
                <div class="d-flex align-items-center">
                    ...
                    <div class="input-group">
                        <input type="hidden" name="author_id" value="{{ $post->author_id }}">
                        <input type="email" name="email" class="form-control" id="subscribeSrArticle"
                            placeholder="Email của bạn" aria-label="Your email" required>
                    </div>
